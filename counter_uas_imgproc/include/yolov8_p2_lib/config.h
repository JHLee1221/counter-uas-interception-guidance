#define USE_FP16
//#define USE_FP32
//#define USE_INT8

// for kari antidrone model. yolov8-p2, s2
const static char* kInputTensorName = "images";
const static char* kOutputTensorName = "output";
const static int kNumClass = 2;
const static int kNumberOfPoints = 17;  // number of keypoints total
const static int kBatchSize = 1;
const static int kGpuId = 0;
const static int kInputH = 1920;
const static int kInputW = 1920;
const static float kNmsThresh = 0.5f;
const static float kConfThresh = 0.2f;
const static float kConfThreshKeypoints = 0.15f;  // keypoints confidence
const static int kMaxInputImageSize = 1920 * 1920;
const static int kMaxNumOutputBbox = 1000;

// Classfication model's number of classes
constexpr static int kClsNumClass = 1000;

// Classfication model's input shape
constexpr static int kClsInputH = 224;
constexpr static int kClsInputW = 224;